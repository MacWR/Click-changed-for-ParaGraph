...
ClipboardWithDiscard::pull(int port)
{
    ...
    if (port == 0) {
        ...
        p->kill();                          //  discard all the packets got from input port 0
        p = NULL;                         
    }else{ //port == 1 
        if ( _dropped ){ 
            _dropped = false;
            p->kill();                      //  discard the packet got from input port 1 while the flag _dropped is true and reset it to false
            p = NULL;
        }else{
            if ( p->anno_u8(7) == (uint8_t)252 ){
                _copied = false;
                p->kill();                  //  discard the packet got from input port 1 
                p = NULL;
            }
            if ( _copied ){
                p = paste(p);
